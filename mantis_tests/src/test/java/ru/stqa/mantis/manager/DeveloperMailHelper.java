...
import ru.stqa.mantis.manager.developermail.GetMessaheResponse;
        ...
        while (System.currentTimeMillis() < start + duration.toMillis()) {
            try {
                ...
                if(response1.result().size() > 0) {
                    var text2 = get(String.format("https://www.developermail.com/api/v1/mailbox/%s/messages/{id}", user.name(), response1.result().get(0)), user.token());
                    var response2 = new ObjectMapper().readValue(text2, GetMessaheResponse.class);
